;;
;; (require 'j-mode)
;; (setf font-lock-keywords-alist (loop for x in font-lock-keywords-alist
;;                                      if (not (eq 'j-mode (car x)))
;;                                      collect x))
;; (funcall 'font-lock-add-keywords nil (loop for v in jc-verbs
;;                                            collect (regexp-quote (car v))))
;; (defun flj ()
;;   (let ((regexes '(("\\[:" 0 font-lock-builtin-face)
;;                    ("\\]" 0 font-lock-warning-face)
;;                    ("\\[" 0 font-lock-warning-face)))
;;         (regexes-2 (sort
;;                     (loop for v in (append jc-verbs jc-adverbs jc-conjunctions) collect (list (regexp-quote (car v))
;;                                                             0 'font-lock-keyword-face))
;;                     (lambda (x y) (> (length (car x)) (length (car y))))
;;                     )
;;                    ))
;;     (font-lock-add-keywords nil (append regexes regexes-2))
